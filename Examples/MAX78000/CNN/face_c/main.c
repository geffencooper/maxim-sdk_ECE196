...
int main(void)
{
  ...
  set_image_dimensions(DIM, DIM); // gets decimated to 28x28
  ...
  while(1)
  {
    ...
    int x = (*((volatile uint32_t *) 0x50404008))/1000;//1100;//900;
    int y = (*((volatile uint32_t *) 0x5040400C))/1000;//1050;//1050;
    int w = (*((volatile uint32_t *) 0x50404010))/1000;//1050;//850;
    int h = (*((volatile uint32_t *) 0x50404014))/1000;//900;//800;
    printf("face: %i\n",face);
    printf("no face: %i\n",no_face);
    printf("x: %i\n",x); // 0,0,0 
    printf("y: %i\n",y); // 0,0,1
    ...
    if(max_i == 0)
    {
      top.x = (x >= 0 && x < DIM) ? x : 0;
      top.y = (y >= 0 && y < DIM) ? y : 0;
      top.w = (top.x+w) <= DIM ? w : DIM-top.x;
      top.h = 2;

      bottom.x = top.x;
      bottom.y = (y+h-2) <= DIM-2 ? y+h-2 : DIM-2;
      bottom.w = top.w;
      bottom.h = 2;
      ...
      left.y = top.y;
      left.w = 2;
      left.h = (left.y+h) <= DIM ? h : DIM-left.y;

      right.x = (x+w) <= DIM-2 ? (x+w): DIM-2;
      right.y = top.y;
      right.w = 2;
      right.h = left.h;

      top.x = DIM-top.x-top.w;
      bottom.x = DIM-bottom.x-bottom.w;
      left.x = DIM-left.x-2;
      right.x = DIM-right.x-2;

      //bottom.y = top.y + left.h-2;

      MXC_TFT_FillRect(&top, 200);
      MXC_TFT_FillRect(&bottom, 200);
      MXC_TFT_FillRect(&left, 200);
      MXC_TFT_FillRect(&right, 200);
    }
