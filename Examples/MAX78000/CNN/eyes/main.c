...
int main(void)
{
  ...
  area_t left_eye = {0, 0, 8, 8};
  area_t right_eye = {0, 0, 8, 8};
  ...
  while(1)
  {
    ...
    int lx = (*((volatile uint32_t *) 0x50404008))/3500;
    int ly = (*((volatile uint32_t *) 0x5040400C))/4400;
    int rx = (*((volatile uint32_t *) 0x50404010))/3500;
    int ry = (*((volatile uint32_t *) 0x50404014))/4400;
    printf("face: %i\n",face);
    printf("no face: %i\n",no_face);
    printf("x: %i\n",lx); // 0,0,0 
    printf("y: %i\n",ly); // 0,0,1
    ...
    if(max_i == 0)
    {
      ...
      left_eye.x = 80-left_eye.x;
      right_eye.x = 80-right_eye.x;

      MXC_TFT_FillRect(&left_eye, 200);
      MXC_TFT_FillRect(&right_eye, 200);
    }
